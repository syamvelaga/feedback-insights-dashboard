...
def process_dataframe_incremental(df, url):
    """Process DataFrame with incremental processing"""
    try:
...
def process_dataframe(df):
    """Original DataFrame processing function (kept for CSV)"""
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                # Use only the primary feedback text for sentiment analysis
                analysis_text = str(feedback_text).strip()
                
                # Perform sentiment analysis (exclude additional comments)
                sentiment_result = analyze_sentiment(analysis_text)
                
                # Check if should be flagged based on primary feedback only
                is_flagged = is_negative_feedback(analysis_text, rating)
